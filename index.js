...
async function triggerMoodAnalysisAndPlay() {
    ...
    // `originalPresetName` (global) should be null here from a previous run's finally block.
    // `currentPresetRestorationRequired` (global) should be false.

    let fetchedOriginalPresetName = null; // Use a clearly named local variable for the fetched value for this specific run

    try {
        const context = getContext();
        if (!context?.chat?.length) {
            console.warn(`${LOG_PREFIX_FUNC} No chat history.`);
            // No AI call, so no preset switch, no restoration needed.
        } else {
            ...
            fetchedOriginalPresetName = getCurrentPresetNameFromUi();
            if (!fetchedOriginalPresetName) {
                console.error(`${LOG_PREFIX_FUNC} Could not determine original preset name. AI suggestion might proceed, but restoration might fail if preset is changed.`);
                originalPresetName = null; // Ensure global is explicitly null if we couldn't get it.
            } else {
                console.log(`${LOG_PREFIX_FUNC} Stored original preset for this run: ${fetchedOriginalPresetName}`);
                // *** KEY FIX: Set the global originalPresetName ***
                originalPresetName = fetchedOriginalPresetName;
            }

            const aiResponseText = await getMusicSuggestionFromAI(chatHistorySnippet);
            // getMusicSuggestionFromAI sets global `currentPresetRestorationRequired` if it successfully switched to MUSIC_PRESET_NAME.
            ...
            if (aiResponseText) {
                ...
                // If AI failed, but preset was switched (i.e., currentPresetRestorationRequired is true),
                // the finally block will still attempt restoration.
            }
        }
    } catch (error) {
        console.error(`${LOG_PREFIX_FUNC} UNEXPECTED ERROR in mood analysis sequence:`, error);
        // The finally block will handle the primary restoration attempt.
        // We can add a specific log here if needed, but the main logic is in finally.
        // Example: if (currentPresetRestorationRequired && originalPresetName) {
        //    console.warn(`${LOG_PREFIX_FUNC} Error occurred after preset switch. Restoration will be attempted in finally block.`);
        // }
    } finally {
        // `originalPresetName` (global) now holds the correct name to restore to (if it was found).
        // `currentPresetRestorationRequired` (global) indicates if getMusicSuggestionFromAI switched the preset.

        if (currentPresetRestorationRequired && originalPresetName) {
            console.log(`${LOG_PREFIX_FUNC} [Finally] Restoring original preset: ${originalPresetName}`);
            ...
            if (!restored) {
                 ...
                 callPopup(`MoodMusic: CRITICAL! Failed to restore preset ${originalPresetName}. Please check manually. Current is: ${getCurrentPresetNameFromUi()}`, "error");
            }
        } else if (currentPresetRestorationRequired && !originalPresetName) {
            // This case means a switch to MUSIC_PRESET_NAME happened (or was intended and currentPresetRestorationRequired got true),
            // but we didn't know what the original preset was.
            console.error(`${LOG_PREFIX_FUNC} [Finally] Preset restoration was required (UI might be on ${MUSIC_PRESET_NAME}), but original preset name was not captured. Please check preset settings manually.`);
            callPopup(`MoodMusic: Error! Switched to ${MUSIC_PRESET_NAME} but original preset was unknown. Please check preset settings. Current: ${getCurrentPresetNameFromUi()}`, "warning");
        }
        // If !currentPresetRestorationRequired, it means either the switch to MUSIC_PRESET_NAME didn't happen,
        // or it failed (in which case getMusicSuggestionFromAI sets currentPresetRestorationRequired to false).
        // In such cases, no restoration is needed from this block.

        isAnalysisInProgress = false;
        currentPresetRestorationRequired = false; // Reset for the next run
        originalPresetName = null; // Reset for the next run
